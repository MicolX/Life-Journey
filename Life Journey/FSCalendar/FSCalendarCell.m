...
@property (nonatomic, strong) NSDateFormatter *dateFormatter;           //改动
...
- (UIColor *)colorForCurrentStateInDictionary:(NSDictionary *)dictionary
{
    ...
    NSArray *journals = [[journalStore sharedStore] allJournals];                              //改动
    for (journal *j in journals) {
        if ([[self.dateFormatter stringFromDate:j.date] isEqualToString:[self.dateFormatter stringFromDate:self.date]]) {
            return dictionary[@(FSCalendarCellStateHasJournals)];
        }
    }
